...
function SnapShotsSection({ projectData }) {
  const [snapshotsCount, setSnapshotsCount] = useState(
    projectData.quickLinks["UI Snapshots"].length > 1 ? 2 : 1,
  );
  return (
    <>
      <div
      ...
      {projectData.quickLinks["UI Snapshots"].length > 2 ? (
        <center>
          <div
            ...
            onClick={() =>
              setSnapshotsCount(
                snapshotsCount === projectData.quickLinks["UI Snapshots"].length
                  ? 2
                  : projectData.quickLinks["UI Snapshots"].length,
              )
            }
          >
            <Image
            ...
            <Link
              ...
              className="
                                 font-medium text-base text-[var(--accent)] underline underline-offset-4
                            "
            >
              {snapshotsCount === projectData.quickLinks["UI Snapshots"].length
                ? "Show Less"
