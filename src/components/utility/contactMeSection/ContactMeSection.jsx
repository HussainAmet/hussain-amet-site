...
function ContactMeSection({ siteData }) {
  return (
    <div
      className="
            border-[2px] border-(--light-white) rounded-[45px] font-aptly-medium-italic
            xl:px-24 xl:py-11
            px-8 py-16
        "
    >
      ...
      >
        <Text
          noref={true}
          text="Contact Me"
          className="xl:text-7xl text-4xl font-aptly-medium-italic xl:w-fit lg:w-fit md:w-fit sm:w-fit"
        />
        <div
          className="
        ...
        >
          {siteData.contacts.map((contact) => (
            <Link
              key={contact.label}
              href={contact.link}
              className="flex gap-4 linkHover"
              target="_blank"
            >
